...
Сама концепция реакта - это, то, что с его помощью можно создавать одностраничные приложения spa
которые ведут себя крайня отзывчиво и выгледят как настоящие программ, они активно взаимодействуют
с пользователем и реагируют на его действия, изменения внешнего вида.

Один из примеров это google mail которая очень удобная в своём использовании, также они должны работать
с серверной строной, уметь получать данные из баз данных и уметь записывать их туда. Для всего этого
там и нужен javaScript язык интерактивности.
Реакт построен на модульной структуре на компонентах, это отдельные кусочки кода, отдельные 
независымые части которыем омжно использовать где угодно, преимущество такого подхода мы знаем и 
собирать их в единное целове можно при помощи чего угодно на подобии web pack и сам web pack.

в любой папке на компьютере зажимаем шифт + правкая кнопка мыши и заходим в PowerShell
там вводим npm install -g create-react-app
далее create-react-app название приложения
заходим в наш проект cd my-app
и запускаем npm start
React уже включает в себе webPack полностью настроенный для работы и babel для использования на более
старых браузерах, также он использует предпроцессор JSX это такая помись javaScripta и html 
которая после рендеринга превращается в вёрстку и вставляеться на вашу страницу

React состоит в осноном из двух элементов 
import React from 'react'; - хранит в себе сам реакт, методы и функции
import ReactDOM from 'react-dom'; - отвечает за создание DOM элементов на нашей странице

Все функции и классы в REACT нужно писать с большой буквы
...
мы отобразили строку на странице, для того чтобы передать переменную в строку используем {} скобки, и никаких косых 
ковычек на не нужно, react всё сделает за нас, так в {} в скобках мы можем использовать функцию или выражение
...
также мы можем передавать наши аргументы через функцию и вызывать их через ReactDom.render
...
ReactDOM.render(<Greet phrase = "World" name="Petr"/>, document.getElementById('root'));

В React обычно компоненты создаються в обьектном стиле будем использовать ООП и создавать классы из нового стандарта
render() метод отвечает за, то, чтобы что-то отобразить на нашей странице 
*/


/*                                Знакомство с CANVAS

Canvas используют для отрисовки графики с большим количеством элементов, у него высокая происводительность
и мы имеем контроль над каждым пикселем. Часто используют для отрисовки полей с большим количеством точек
для браузерных игра например, так же для геометрических эффектов изображения, и его можно использовать
для встраивания видео в страницу и создание полноценного плеера,а ещё для потокового видео - например
изображение берёться с камеры и прямом эфире накладываеться ушки, шапочки, антенки, как в масках. 

canvas как обьект имеет все параметры DOM дерева + прототип HTMLCanvasElement, в котором есть метод
getContext() - с помощью него мы получим контекст канваса и сможем работать с графикой 

const context = canvas.getContext(); - это метод принимает первым параметром строку в которой мы указываем
какой контекст мы хотим получить, есть четыре значения.
2d - графика
webJL - необходимы для трёх мерной графики
webJL2
bitMapRender - это для работы с растровыми изображениями 

Мы будем работать с 2d и только по базе.

context.fillRect(); метод создаёт закрашенный прямоугольник и принимает 4 параметра 
1 - начальная координата по горизонтале х
2 - начальная координата по вертикале у
3 - ширина
4 - высота

context.fillStyle = '#bb334';
context.fillStyle = 'rgba(50,255,50, 0.3)'; последний параметр можно установить прозрачность
context.fillStyle = 'hsl(200, 70%, 70%)';

context.strokeRect(20, 20, 100, 100); методом можем нарисовать незакрашенный прямоугольник

context.strokeStyle = 'rgb(50, 155, 50)'; - меняем стиль нашего прямоугольника
context.strokeRect(140, 20, 100, 100);

Есть ещё метод котоорый рисует полностью прозрачный квадрат
context.clearRect(20, 20, 100, 100); - это квадрат можно нагладывать на элемены на страницы и они станут
прозрачными  context.clearRect(45, 45, 50, 50); можно сделать бублик - квадрат с пустым центром

также мы можем указать градиент

создаём градиент, и указываем в первых двух начальную точку и в последниз 2х параметрах конечную точку 
градиента
const gradient = context.createLinearGradient(20, 20, 120, 120);

дальше нам нужно добавить цвет - это методо addColorStop
1- параметром указываем какой мы цвет указываем начальный или конечный.
это указываеться с помощью чисел 0 это начальное число, 1 это конечный цвет.
2- параметр указываем цвет

gradient.addColorStop(0, 'hsl(250, 70%, 70%)');
указываем второй цвет
gradient.addColorStop(1, 'hsl(0, 70%, 70%)');

дальше мы наш цвет должны применить к свойству fillStyle 

context.fillStyle = gradient;
context.fillRect(20, 20, 100, 100);

получили нежный переход

Цвета мы можем пережать в любом формате.

Также есть заливка по кругу, вместо createLinearGradient мы прописываем createRadialGradient, 
единственное он принимает 6 параметров.
Я хочу начать из центра и мне необходимо указать первый кружочек откуда это всё пойдёт.
первый кружочек у нас будет ровно по середине это 70 на 70 и укажем 0 пикселей он просто там будет и
от него мы начнём делать градиент, дальше мы будем указывать второй круг, тоже где он у нас находиться
это 70 на 70, 70 площадью это конец кнуга.

const gradient = context.createRadialGradient(70, 70, 0, 70, 70, 70);
применяем.
gradient.addColorStop(0, 'hsl(250, 70%, 70%)');
gradient.addColorStop(1, 'hsl(0, 70%, 70%)');
...
const gradient = context.createRadialGradient(70, 70, 30, 70, 70, 70); сделав так у нас будет больше
первого цвета чем конечного, если указать размер радиуса у второго круга, например 50, то у нас раньше
произойдёт переход ко второму цвету

createRadialGradient(70, 70, 30, 70, 70, 70)
1 - координата по горизонтале х начальной окружности
2 - координата по горизонтале у начальной окружности
3 - радиус этой окружности
4 - координата по горизонтале х конечной окружности
5 - координата по горизонтале у конечной окружности
6 - радиус этой окружности

Помимо прямоугольников и градиентов, мы можем ещё рисовать и сложные фигуры, они рисуються с помощью
контуров, начало такой фигуры задаёться с помощью метода beginPath();

context.beginPath(); мы сейчас обозначили начало контура и всё что будет идти после него будет считаться 
одим единым контуром, но а чтобы его увидиться на необходимо его обвести, это делаеться методом


и теперь между двумя методами я могу перемещаться с помощью метода mathTool(); или рисовать линии lineTo();
указать координаты куда я хочу перместиться при этом начертить за собой линию
context.stroke();
...
Мы на рисовали фигуру 300 на 300 пикселей а канвас у нас позволяет использовать только по умолчанию 
300 на 150. Необходимо поменять его высоту, чтобы увидить весь наш элемент.
Чтобы поменять размеры нашего канваса нашего элемнета на странице нам необходимо обратиться 
к html attributes, таким как width и height

<canvas id="canvas" width="300" height="300"></canvas> 

чтобы поменять толщену линии используем команду 
context.lineWidth = '2'; в параметрах указываем толщину в пикселях, обьявляем метод до метода context.stroke();
так же мы можем поменять цвет  context.strokeStyle
...
так же мы можем задавать окружности и дуги.
Передвигаемся в центр нашего элемента context.moveTo(150, 150); И мы можем предстваить что наша 
кисть находиться в центре нашего элемента
и чтобы нарисовать окружность используем метод context.arc()
этот метод принимает 6 параметров
1. координаты центра дуги по х
2. координаты центра дуги по у
3. радиус нашего круга или дуги
4. угол начала отрисовки дуги 
5. угол конца отрисовки (все угла в санвас измеряються в радианах)
поэтому мы можем написать чисто Math.Pi и следующим парметром мы указываем true или false это будет 
значение которое сообщяет по часовой стрелке будет рисоваться круг или против часовой. Если это круг
это неважно, а если дуга то важно

context.arc(150, 150, 25, 0, Math.PI, false); в конце Math.PI 

избавляемся от ручки передвигаясь на 20 пикселей вправа и убножая число пи на 2 змыкаем нашу окружность
context.arc(150, 150, 25, 0, Math.PI * 2, false);


Функция конвертирует из градусов в радианы
const angle = (degrees) => (Math.PI / 180) * degrees;

чтобы воспользоваться можем указать нашу функцию в параметре и передать туда градусы 
context.arc(150, 150, 25, 0, angle(180), false);

Также мы можем рисовтаь плавные углы различной загруглённоести 
для этого есть метод arcTo(); он принимает 5 параметров
1. Параметр начальной точки х
2. Параметр начальной точки у
3. Параметр конечной точки х
4. Параметр конечной точки у
5. радиус
...
context.arc(150, 150, 25, 0, angle(), true);
...
Так же мы можем нарисовать кривую Безье 
для этого есть метод context.bezierCurveTo который принимает 6 парметров, это 3 точки.
1.  коррдинаты 1 точки
2.  коррдинаты 1 точки
3.  коррдинаты 2 точки
4.  коррдинаты 2 точки
5.  коррдинаты 3 точки
6.  коррдинаты 3 точки

context.bezierCurveTo(100, 200, 200, 200, 200, 100);
context.bezierCurveTo(200, 0, 200, 200, 300, 100);

В канвас можно списать ещё и текст - метод для этого context.fillText(); принимает 
1. любой текст
2. координаты х
3. координаты у
4. ширина

context.fillText('How are you?', 50, 50, 200);
измеим размеры шрифта и поменяем шрифт
context.font = '30px Sans-serif'; - тут мы можем указать всё как и в css
...
context.shadowBlur = 3;
context.shadowColor = 'red';
context.fillText('How are you?', 20, 50, 200);

создаём ещё текст и с помощью метода rotate() повернём его
context.shadowColor = 'orange';
context.fillStyle = 'blue';
context.rotate(angle(10));
context.fillText('GloAcademy', 210, 50, 200);

с помощью метода save(); мы можем сохранять текущее состояние элементов и взирая на дальнейший код
если мы укажем rotate в середине кода то все последующие элементы после исполняться под углом, сохранив
состояние ранее и при помози метода restore(); мы можем восстановить сохранённое состояние для 
элементов находящихся ниже.

Анимация в канвас реализуеться также как и в javaScript, с помощью setTimeout, setInterval и на самом
деле предпочтительнее requastAnimationFrame.

Создали свою анимацию, где кубик отбиваеться от угла к углу
...
Также мы можем реализовать Paint на CANVAS 
для этого нам нужно работать с обработчиками событий
...
представим что наша программа это автомобиль, она делиться на модули кузов двигатель и электронику тоже можно
разделить на модули, типо кондиционер, печка. Эти многие модули могут друг без друга работать, например, если
у машине не будет двигателя, но мы всё также сможем слушать музыку.
Концепция модулей чем то похожа на функцию, это переиспользуемая часть кода, она независимая и самодостаточная 
если модуль калькулятор к примеру убрать с нашего сайта, то таймер и слайдер также будт работать, это ни как 
неповлияет на остальную часть кода. 
Модули помогают обстрагировать код от основной часть программы, делать код независемым, так же он позволяет
икапсулировать код, скрывать его внутри модуля и переиспользовать код, например тотже самый слайдер мы
можем использовать на сайте два - три раза или использовать его в других проектах, так же модули 
обеспечивают чистату глобального пространства,, наши переменные и функции внутри модуля остаються внутри 
модуля и не видны в глобальное области видимости. Модуль позволяют держать глобальное пространство в чистоте
и предотвращают конфликт имён. Если у нас переменные с один именем но в разных модулях, они даже друг о друге
не будут знать.


Бывает так, унас есть обьект appData, у нём есть какие-то данные или функции не важно, нам например говорит
шеф чтобы подключили сторонюю библиотеку, мы подключаем и оказываеться что там есть тоже обьект appData
со своим функциями и переменными, но у нас уже конфликт имён.

Делаем модуль и заворачиваем наш appData в ( ) скобки и делаем из неё самовызывающую функцию
...
Создание анонимной и самовызывающейся функцией это самый популярный спосбо создание модулей.


Следующий способ это мы можем сам модуль записать в переменную и возвращать методы, которые будут доступны
внешне, называеться этот способо использование обьектного интерфейса.

const car = (function(){
  const hidden = 'Я приватная переменная';
  const secret = () => {
    console.log(hidden);
  }
}());

Наши переменные hidden и secret не видны снаружи и мы никак не можем использовать во вне, но наш модуль
может возвращать обьект этоже фукнция.
...
Модуль может возваращать обьект и обьект может содержать собственные функции
Если мы вызовим нашу функцию car() то получим ошибку, что car is not a function.
Давайте посмотри что такое car

console.log('car: ', car); и увидим что car это обьект содержащий метод run, также мы должны заметить, что
переменные secret и hidden мы не видем, они у нас скрыты внутри.
Давайте вызовим эту функцию
car.run(); получим врум врум... 

Мы скрыли наши переменные, мы можем выполнть здесь любые действия, а возвращать только то что мы захотим.
Но внутренний интерфейс мы также можем использовать.
вместо строки вывести переменную hidden или в return вызвать функцию secret
...
ещё один варинат это присвоить нашей функици свойства и методы к внутренним переменным.
И мы приватные свойства сделали публичными, но воздействовать на них и как-то поменять мы не можем, они
полностью защищены внутри этого интерфейста внутреннего модуля
const car = (function(){
  const hidden = 'Я приватная переменная';
  const secret = () => {
    console.log(hidden);
...
В больших проектах JS дробят на модули на отдельные файлы, представле что в однмо файле было 20 или 50тысяч
строк, пойди разберись, что да куда, поэтому лучшим решением будет разбить проект на маленькие части и есть 
несколько подходов опеделения модулей, common JS и другие...

У нас есть 2 модуля, 2 js файла 
moduleOne.js и moduleTwo.js 
в каждем есть своя функция.
...
и теперь эти 2 модуля нам необходимо соединить вместе.
Мы их будем экспортировать в один проект, для этого нам необходимо 
в каждом из этих модулей прописать экспорт.

module.expotrs = moduleOne;
module.expotrs = moduleTwo;

далее в файле например index.js нам необходим принять эти файлы, создаём переменную (она может иметь
такойже имя или другой) 
и чтобы подключить есть функция require( ) и в ковычках мы указываем файл из которого мы хотим произвести 
импорт причём расширение .js писать не обязательно
...
moduleOne();
moduleTwo();
теперь в этих переменный будет содержаться всё то что мы экспортировали, но нам необходимо собрать эти 3 файла 
в один бандл. для этого нам понадобить изучить сборщик

1 из них browserify (http://browserify.org/)
он собирает модули - для его установки необходимо прописать команду npm install -g browserify
флаг -g даёт нам возможность вызвать команду browserify в любой точке кода.

для того чтобы собрать один файл bundle.js для этого в терминали необходимо прописать команду
указать главный файл main.js дальше ставим ключь тире -о и указать имся файла который будем собирать

browserify main.js -o bundle.js

это делаеться так: после browserify указываем путь к файлу которыйх хотим взять и после -о указываем куда
хотим положить
browserify 3dGlo/index.js -o testFolder/main.js

затем в html подключаем созданный файл.
через консоле и sourse слева в колонке можем увидеться подключенный созданный бандл




Соверменный синтаксис import export
с помощью которыйх мы можем экспортировать данные 
...
Так же мы можем эхпортировать функции таким образом
...
Так же мы можем экспортировать классы 
export class Car {
  constructor(brand){
    this.brand = brand;
  }
...
export { a, b, run, eat, Car };

Так же мы можем переменнов при экспорте
...
таким образом мы получаем локальные переменные со значением из импортированного модуля
и также можем переименновывать

import { one as num, two as towNum } from './moduleone';
если мы хотим импортировать все данные сразу мы можем указать звёздочку *, после указать имя обьекта
куда это всё мы положем

import * as obj from './moduleone';

console.log(obj.one);
console.log(obj.two);
или даже можем потов вызывать функцию обращаеся через наш обьекст
obj.run();  

У нас есть возможность экспортировать по default дефолту 
ещё момент что экспорт не работает на function exprtation
он может экпортировать только function dicloration
вот так
...
Если нужно передать в запрос параметры в файлы cookie то при использование fetch нам нужно открыть доступ к отправке
с помощью свойства credentials: 'include' - это передаёт параметры индификации и указал include вы уже будите 
проходить проверку подлинности, только проходить с помощью cookie.
*/
...
кнопке.
Рассмотри раб��ту без делегирования!
...
если мы передадим наш массив таким ��бразом 
...
ключевого слово let a = 5; т.е без обьявления. всегда указывать полный с��нтаксис.
